...
public class PathStairs extends BaseStairs {
    ...
    public PathStairs(BlockState defaultState, Settings settings) {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        if(!this.canPlaceAt(ctx.getWorld().getBlockState(ctx.getBlockPos()), ctx.getWorld(), ctx.getBlockPos())){
            return Block.pushEntitiesUpBeforeBlockChange(this.getDefaultState(), ModBlocks.DIRT.getBlock(ModBlocks.BlockType.STAIRS).getPlacementState(ctx), ctx.getWorld(), ctx.getBlockPos());
        }
