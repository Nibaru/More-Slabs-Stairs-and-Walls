...
public class DirtWall extends BaseWall {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if(blockState.getMaterial().isSolid() && !(blockState.getBlock() instanceof FenceGateBlock)) return ActionResult.PASS;

        if (!world.isClient) {
            world.setBlockState(pos, ModBlocks.DIRT_PATH.getBlock(ModBlocks.BlockType.WALL).getStateWithProperties(state));
