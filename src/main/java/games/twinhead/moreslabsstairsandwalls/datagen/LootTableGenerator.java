...
public class LootTableGenerator extends SimpleFabricLootTableProvider {
    ...
    public void accept(BiConsumer<Identifier, LootTable.Builder> identifierBuilderBiConsumer) {
        for (ModBlocks block: ModBlocks.values()) {
            if(block.hasSlab())
                identifierBuilderBiConsumer.accept(new Identifier(MoreBuildingBlocks.mod_id, "blocks/" + block.toString().toLowerCase() + "_slab"), BlockLootTableGenerator.slabDrops(block.getSlabBlock()));
    ...
    public void addBlock(BiConsumer<Identifier, LootTable.Builder> identifierBuilderBiConsumer, String name, Block block){
        identifierBuilderBiConsumer.accept(new Identifier(MoreBuildingBlocks.mod_id, "blocks/" + name), BlockLootTableGenerator.drops(block, block.asItem(), ConstantLootNumberProvider.create(1.0F)));
    }
}
